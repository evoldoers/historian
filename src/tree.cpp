...
void Tree::buildByNeighborJoining (const vguard<string>& nodeName, const vguard<vguard<TreeBranchLength> >& distanceMatrix) {
  ...
  while (true)
      ...
      for (auto ni : activeNodes)
	{
	  ...
	  LogThisAt(2,"Distance correction for node " << ni << " is " << avgDist[ni] << endl);
	}
	...
	for (node_j_p = node_i_p, ++node_j_p; node_j_p != activeNodes_end_p; ++node_j_p)
	  {
	    ...
	    LogThisAt(2,"Compensated distance from node " << *node_i_p << " to node " << *node_j_p << " is " << compensatedDist << endl);
      ...
      if (d_jk < 0)
	{
      ...
      node[min_j].parent = k;
      node[min_j].d = d_jk;
      LogThisAt(3,"Joining nodes " << min_i << " and " << min_j << " to common ancestor " << k << " (branch lengths: " << k << "->" << min_i << " = " << d_ik << ", " << k << "->" << min_j << " = " << d_jk << ")" << endl);
